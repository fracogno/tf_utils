...
class CNN(tf.keras.Model):
    ...
    def __init__(self, filters, kernel_size, strides=1, padding="SAME", output_padding=None, data_format=None, dilation_rate=1, groups=1, use_bias=True,
        ...
        self.parts = []
        if not up:
            self.parts.append(tf.keras.layers.Conv3D(filters=filters, kernel_size=kernel_size, strides=strides, padding=padding,
                                                     data_format=data_format, dilation_rate=dilation_rate, groups=groups, activation=None,
            ...
            self.parts.append(tf.keras.layers.Conv3DTranspose(filters=filters, kernel_size=kernel_size, strides=strides, padding=padding, output_padding=output_padding,
                                                              data_format=data_format, dilation_rate=dilation_rate, activation=None,
                                                              use_bias=use_bias, kernel_initializer=kernel_initializer, bias_initializer=bias_initializer,
                                                              kernel_regularizer=kernel_regularizer, bias_regularizer=bias_regularizer, activity_regularizer=activity_regularizer,
                                                              kernel_constraint=kernel_constraint, bias_constraint=bias_constraint))
    ...
    def summary(self, input_shape):
        ...
        tf.keras.utils.plot_model(model, to_file='CNN.png', show_shapes=True, expand_nested=True)
        model.summary(line_length=200)


if __name__ == "__main__":
    block = CNN(32, 3, batch_norm=True, dropout=0.3, activation=tf.keras.layers.LeakyReLU)
    block.summary((64, 64, 64, 3))
